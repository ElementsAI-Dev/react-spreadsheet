...
import reducer, { INITIAL_STATE, hasKeyDownHandler } from "../state/reducer";
...
export type Props<CellType extends Types.CellBase> = {
  ...
  /**
   * Virtualization configuration for large spreadsheets.
   * When enabled, only visible cells are rendered for better performance.
   * Requires specifying height and width for the virtualized container.
   */
  virtualization?: Types.VirtualizationConfig;
};
...
const Spreadsheet = <SpreadsheetRef, CellType extends Types.CellBase>(
  ...
  // Virtualized table node (only created when virtualization is enabled)
  const virtualizedTableNode = React.useMemo(() => {
    ...
    return (
      <VirtualizedTable
        ...
        columnWidth={virtualization.columnWidth}
        rowIndicatorWidth={virtualization.rowIndicatorWidth}
        ...
        overscanRowCount={virtualization.overscanRowCount}
        overscanColumnCount={virtualization.overscanColumnCount}
      />
    );
  }, [
    virtualization,
  ...
  // Regular table node (only created when virtualization is disabled)
    ...
    () => (
      <div
        ...
        className={classNames("Spreadsheet", className, {
          ...
          "Spreadsheet--virtualized": virtualization?.enabled,
        })}
        ...
        onBlur={handleBlur}
      >
        {virtualization?.enabled ? virtualizedTableNode : tableNode}
        {activeCellNode}
        <Selected />
        <Copied />
      </div>
