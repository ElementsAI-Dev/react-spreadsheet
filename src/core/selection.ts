...
export class EntireWorksheetSelection extends EntireSelection {
  private _rangeCache = new WeakMap<Matrix.Matrix<unknown>, PointRange>();

  toRange(data: Matrix.Matrix<unknown>): PointRange {
    let cached = this._rangeCache.get(data);
    if (!cached) {
      cached = getMatrixRange(data);
      this._rangeCache.set(data, cached);
    }
    return cached;
  }
