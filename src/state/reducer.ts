  ...
  switch (action.type) {
    ...
    case Actions.PASTE: {
      ...
      if (selectedRange && copiedSize.rows === 1 && copiedSize.columns === 1) {
        ...
        const updates: Array<[Point.Point, Types.CellBase | undefined]> = [];
        for (const point of selectedRange) {
          ...
          updates.push([point, cell]);
        }
        // Use batch operation for better performance
        const newData = Matrix.setMultiple(updates, baseData);
      ...
      const updates: Array<[Point.Point, Types.CellBase | undefined]> = [];
      const unsets: Point.Point[] = [];
      const commits: Types.CommitChanges = [];
      ...
      for (const [point, cell] of Matrix.entries(copied)) {
        ...
        if (state.cut && state.copied) {
          ...
          unsets.push(prevPoint);
          commits.push({ prevCell: cell || null, nextCell: null });
        }

        if (Matrix.has(nextPoint, paddedData)) {
          const currentCell = Matrix.get(nextPoint, paddedData) || null;
          commits.push({
      ...
      // Apply unsets first, then updates in batch
      let newData = paddedData;
      if (unsets.length > 0) {
        const unsetUpdates = unsets.map((p): [Point.Point, undefined] => [
          p,
          undefined,
        ]);
        newData = Matrix.setMultiple(unsetUpdates, newData);
      }
      newData = Matrix.setMultiple(updates, newData);
...
function clear(state: Types.StoreState): Types.StoreState {
  ...
  for (const point of selectedRange) {
    ...
    updates.push([point, clearedCell]);
  }

  // Use batch operation for better performance
  const newData = Matrix.setMultiple(updates, state.model.data);
